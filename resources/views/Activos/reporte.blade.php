...
<div class="container-fluid shadow-sm p-3 mb-5 bg-white rounded">
   ...
   <div class="col-8">
        <div class="input-group mb-3">
            <input type="text" class="form-control" id="texto" placeholder="Ingresar coincidencia" aria-label="Recipient's username" aria-describedby="basic-addon2">
            <select class="form-select form-control" aria-label="Default select example">
                <option selected>Seleccione un filtro</option>
                <option value="UsuarioAsig">Usuario Asigado</option>
                <option value="Oficina">Oficina</option>
                <option value="NombreActivo">Tipo de activo</option>
            </select>
        </div>
   </div>
...
<div class="table-responsive-lg">
                    <table class="table table-striped table-hover" id="d">
                        <thead>
                           <tr> 
                                <th width="40px">ID</th>
                                <th>Codigo</th>
                        ...
                        <tbody>
                            <div id="d"><!-- @foreach($activos as $activo) -->
                                <tr>
                                    <td>{{$activo->id}}</td>
                                    <td>{{$activo->Codigo}}</td>
...
<script>
    ...
    document.getElementById("texto").addEventListener("keyup", () =>{
        ...
        {
            fetch(`/buscar_activos?texto=${document.getElementById("texto").value}`,{method:'get'})
            .then(response => response.text())
            .then(html =>{document.getElementById("d").innerHTML = html})
        }else{
